"""
KüçükÇiftlik Park — https://www.kucukciftlikpark.com.tr
"""
import logging
from scrapers.base import fetch, infer_genres

logger = logging.getLogger(__name__)
BASE = "https://www.kucukciftlikpark.com.tr"
...
def scrape() -> list[dict]:
    ...
    events = []
    for card in soup.select(".event-card, article, .event-item, .concert-item, .etkinlik"):
        try:
            title_el = card.select_one("h2, h3, h4, .title, .concert-title, .event-title")
            ...
            date_el = card.select_one("time, .date, .tarih, [class*='date']")
            ...
            logger.error(f"KüçükÇiftlik parse error: {e}")

    logger.info(f"KüçükÇiftlik Park: found {len(events)} events")
    return events
