"""
IF Performance Hall — https://www.ifperformancehall.com
"""
import logging
from scrapers.base import fetch, infer_genres

logger = logging.getLogger(__name__)
BASE = "https://www.ifperformancehall.com"
...
def scrape() -> list[dict]:
    ...
    events = []
    for card in soup.select(".event-card, article, .event-item, .concert, .show"):
        try:
            title_el = card.select_one("h2, h3, h4, .title, .event-title, .concert-title")
    ...
    logger.info(f"IF Performance Hall: found {len(events)} events")
    return events
